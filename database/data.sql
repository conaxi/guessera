INSERT INTO countries (name, lat, lng) VALUES
('Afghanistan',33.0, 65.0),
('Åland Islands',60.116667, 19.9),
('Albania',41.0, 20.0),
('Algeria',28.0, 3.0),
('American Samoa',-14.33333333, -170.0),
('Andorra',42.5, 1.5),
('Angola',-12.5, 18.5),
('Anguilla',18.25, -63.16666666),
('Antarctica',-74.65, 4.48),
('Antigua and Barbuda',17.05, -61.8),
('Argentina',-34.0, -64.0),
('Armenia',40.0, 45.0),
('Aruba',12.5, -69.96666666),
('Australia',-27.0, 133.0),
('Austria',47.33333333, 13.33333333),
('Azerbaijan',40.5, 47.5),
('Bahamas',24.25, -76.0),
('Bahrain',26.0, 50.55),
('Bangladesh',24.0, 90.0),
('Barbados',13.16666666, -59.53333333),
('Belarus',53.0, 28.0),
('Belgium',50.83333333, 4.0),
('Belize',17.25, -88.75),
('Benin',9.5, 2.25),
('Bermuda',32.33333333, -64.75),
('Bhutan',27.5, 90.5),
('Bolivia (Plurinational State of),',-17.0, -65.0),
('Bonaire, Sint Eustatius and Saba',12.15, -68.266667),
('Bosnia and Herzegovina',44.0, 18.0),
('Botswana',-22.0, 24.0),
('Bouvet Island',-54.43333333, 3.4),
('Brazil',-10.0, -55.0),
('British Indian Ocean Territory',-6.0, 71.5),
('Virgin Islands (British),',18.431383, -64.62305),
('Virgin Islands (U.S.),',18.34, -64.93),
('Brunei Darussalam',4.5, 114.66666666),
('Bulgaria',43.0, 25.0),
('Burkina Faso',13.0, -2.0),
('Burundi',-3.5, 30.0),
('Cambodia',13.0, 105.0),
('Cameroon',6.0, 12.0),
('Canada',60.0, -95.0),
('Cabo Verde',16.0, -24.0),
('Cayman Islands',19.5, -80.5),
('Central African Republic',7.0, 21.0),
('Chad',15.0, 19.0),
('Chile',-30.0, -71.0),
('China',35.0, 105.0),
('Christmas Island',-10.5, 105.66666666),
('Cocos (Keeling), Islands',-12.5, 96.83333333),
('Colombia',4.0, -72.0),
('Comoros',-12.16666666, 44.25),
('Congo',-1.0, 15.0),
('Congo (Democratic Republic of the),',0.0, 25.0),
('Cook Islands',-21.23333333, -159.76666666),
('Costa Rica',10.0, -84.0),
('Croatia',45.16666666, 15.5),
('Cuba',21.5, -80.0),
('Curaçao',12.116667, -68.933333),
('Cyprus',35.0, 33.0),
('Czech Republic',49.75, 15.5),
('Denmark',56.0, 10.0),
('Djibouti',11.5, 43.0),
('Dominica',15.41666666, -61.33333333),
('Dominican Republic',19.0, -70.66666666),
('Ecuador',-2.0, -77.5),
('Egypt',27.0, 30.0),
('El Salvador',13.83333333, -88.91666666),
('Equatorial Guinea',2.0, 10.0),
('Eritrea',15.0, 39.0),
('Estonia',59.0, 26.0),
('Ethiopia',8.0, 38.0),
('Falkland Islands (Malvinas),',-51.75, -59.0),
('Faroe Islands',62.0, -7.0),
('Fiji',-18.0, 175.0),
('Finland',64.0, 26.0),
('France',46.0, 2.0),
('French Guiana',4.0, -53.0),
('French Polynesia',-15.0, -140.0),
('French Southern Territories',-49.25, 69.167),
('Gabon',-1.0, 11.75),
('Gambia',13.46666666, -16.56666666),
('Georgia',42.0, 43.5),
('Germany',51.0, 9.0),
('Ghana',8.0, -2.0),
('Gibraltar',36.13333333, -5.35),
('Greece',39.0, 22.0),
('Greenland',72.0, -40.0),
('Grenada',12.11666666, -61.66666666),
('Guadeloupe',16.25, -61.583333),
('Guam',13.46666666, 144.78333333),
('Guatemala',15.5, -90.25),
('Guernsey',49.46666666, -2.58333333),
('Guinea',11.0, -10.0),
('Guinea-Bissau',12.0, -15.0),
('Guyana',5.0, -59.0),
('Haiti',19.0, -72.41666666),
('Heard Island and McDonald Islands',-53.1, 72.51666666),
('Vatican City',41.9, 12.45),
('Honduras',15.0, -86.5),
('Hungary',47.0, 20.0),
('Hong Kong',22.25, 114.16666666),
('Iceland',65.0, -18.0),
('India',20.0, 77.0),
('Indonesia',-5.0, 120.0),
('Ivory Coast',8.0, -5.0),
('Iran (Islamic Republic of),',32.0, 53.0),
('Iraq',33.0, 44.0),
('Ireland',53.0, -8.0),
('Isle of Man',54.25, -4.5),
('Israel',31.5, 34.75),
('Italy',42.83333333, 12.83333333),
('Jamaica',18.25, -77.5),
('Japan',36.0, 138.0),
('Jersey',49.25, -2.16666666),
('Jordan',31.0, 36.0),
('Kazakhstan',48.0, 68.0),
('Kenya',1.0, 38.0),
('Kiribati',1.41666666, 173.0),
('Kuwait',29.5, 45.75),
('Kyrgyzstan',41.0, 75.0),
('Lao People`s Democratic Republic',18.0, 105.0),
('Latvia',57.0, 25.0),
('Lebanon',33.83333333, 35.83333333),
('Lesotho',-29.5, 28.5),
('Liberia',6.5, -9.5),
('Libya',25.0, 17.0),
('Liechtenstein',47.26666666, 9.53333333),
('Lithuania',56.0, 24.0),
('Luxembourg',49.75, 6.16666666),
('Macao',22.16666666, 113.55),
('North Macedonia',41.83333333, 22.0),
('Madagascar',-20.0, 47.0),
('Malawi',-13.5, 34.0),
('Malaysia',2.5, 112.5),
('Maldives',3.25, 73.0),
('Mali',17.0, -4.0),
('Malta',35.83333333, 14.58333333),
('Marshall Islands',9.0, 168.0),
('Martinique',14.666667, -61.0),
('Mauritania',20.0, -12.0),
('Mauritius',-20.28333333, 57.55),
('Mayotte',-12.83333333, 45.16666666),
('Mexico',23.0, -102.0),
('Micronesia (Federated States of),',6.91666666, 158.25),
('Moldova (Republic of),',47.0, 29.0),
('Monaco',43.73333333, 7.4),
('Mongolia',46.0, 105.0),
('Montenegro',42.5, 19.3),
('Montserrat',16.75, -62.2),
('Morocco',32.0, -5.0),
('Mozambique',-18.25, 35.0),
('Myanmar',22.0, 98.0),
('Namibia',-22.0, 17.0),
('Nauru',-0.53333333, 166.91666666),
('Nepal',28.0, 84.0),
('Netherlands',52.5, 5.75),
('New Caledonia',-21.5, 165.5),
('New Zealand',-41.0, 174.0),
('Nicaragua',13.0, -85.0),
('Niger',16.0, 8.0),
('Nigeria',10.0, 8.0),
('Niue',-19.03333333, -169.86666666),
('Norfolk Island',-29.03333333, 167.95),
('Korea (Democratic People`s Republic of),',40.0, 127.0),
('Northern Mariana Islands',15.2, 145.75),
('Norway',62.0, 10.0),
('Oman',21.0, 57.0),
('Pakistan',30.0, 70.0),
('Palau',7.5, 134.5),
('Palestine, State of',31.9, 35.2),
('Panama',9.0, -80.0),
('Papua New Guinea',-6.0, 147.0),
('Paraguay',-23.0, -58.0),
('Peru',-10.0, -76.0),
('Philippines',13.0, 122.0),
('Pitcairn',-25.06666666, -130.1),
('Poland',52.0, 20.0),
('Portugal',39.5, -8.0),
('Puerto Rico',18.25, -66.5),
('Qatar',25.5, 51.25),
('Republic of Kosovo',42.666667, 21.166667),
('Réunion',-21.15, 55.5),
('Romania',46.0, 25.0),
('Russian Federation',60.0, 100.0),
('Rwanda',-2.0, 30.0),
('Saint Barthélemy',18.5, -63.41666666),
('Saint Helena, Ascension and Tristan da Cunha',-15.95, -5.7),
('Saint Kitts and Nevis',17.33333333, -62.75),
('Saint Lucia',13.88333333, -60.96666666),
('Saint Martin (French part),',18.08333333, -63.95),
('Saint Pierre and Miquelon',46.83333333, -56.33333333),
('Saint Vincent and the Grenadines',13.25, -61.2),
('Samoa',-13.58333333, -172.33333333),
('San Marino',43.76666666, 12.41666666),
('Sao Tome and Principe',1.0, 7.0),
('Saudi Arabia',25.0, 45.0),
('Senegal',14.0, -14.0),
('Serbia',44.0, 21.0),
('Seychelles',-4.58333333, 55.66666666),
('Sierra Leone',8.5, -11.5),
('Singapore',1.36666666, 103.8),
('Sint Maarten (Dutch part),',18.033333, -63.05),
('Slovakia',48.66666666, 19.5),
('Slovenia',46.11666666, 14.81666666),
('Solomon Islands',-8.0, 159.0),
('Somalia',10.0, 49.0),
('South Africa',-29.0, 24.0),
('South Georgia and the South Sandwich Islands',-54.5, -37.0),
('Korea (Republic of),',37.0, 127.5),
('Spain',40.0, -4.0),
('Sri Lanka',7.0, 81.0),
('Sudan',15.0, 30.0),
('South Sudan',7.0, 30.0),
('Suriname',4.0, -56.0),
('Svalbard and Jan Mayen',78.0, 20.0),
('Swaziland',-26.5, 31.5),
('Sweden',62.0, 15.0),
('Switzerland',47.0, 8.0),
('Syrian Arab Republic',35.0, 38.0),
('Taiwan',23.5, 121.0),
('Tajikistan',39.0, 71.0),
('Tanzania, United Republic of',-6.0, 35.0),
('Thailand',15.0, 100.0),
('Timor-Leste',-8.83333333, 125.91666666),
('Togo',8.0, 1.16666666),
('Tokelau',-9.0, -172.0),
('Tonga',-20.0, -175.0),
('Trinidad and Tobago',11.0, -61.0),
('Tunisia',34.0, 9.0),
('Turkey',39.0, 35.0),
('Turkmenistan',40.0, 60.0),
('Turks and Caicos Islands',21.75, -71.58333333),
('Tuvalu',-8.0, 178.0),
('Uganda',1.0, 32.0),
('Ukraine',49.0, 32.0),
('United Arab Emirates',24.0, 54.0),
('United Kingdom of Great Britain and Northern Ireland',54.0, -2.0),
('United States of America',38.0, -97.0),
('Uruguay',-33.0, -56.0),
('Uzbekistan',41.0, 64.0),
('Vanuatu',-16.0, 167.0),
('Venezuela (Bolivarian Republic of),',8.0, -66.0),
('Vietnam',16.16666666, 107.83333333),
('Wallis and Futuna',-13.3, -176.2),
('Western Sahara',24.5, -13.0),
('Yemen',15.0, 48.0),
('Zambia',-15.0, 30.0),
('Zimbabwe',-20.0, 30.0);